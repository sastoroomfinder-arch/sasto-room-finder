...
async function savePropertyAlert(){
 const name=$("alertName")?.value.trim()||"",phone=$("alertPhone")?.value.trim()||"",email=$("alertEmail")?.value.trim()||"",location=$("alertLocation")?.value.trim()||"",typeV=$("alertType")?.value||"",minPrice=$("alertMinPrice")?.value.trim()||"",maxPrice=$("alertMaxPrice")?.value.trim()||"";
 if(!name||(!phone&&!email)){alert("Please enter your name and either a phone/WhatsApp number or email.");return}
 if(!location&&!typeV&&!minPrice&&!maxPrice){alert("Choose at least one property alert condition.");return}
 const payload={name,phone:phone||null,email:email||null,location:location||null,property_type:typeV||null,min_price:alertNum(minPrice),max_price:alertNum(maxPrice),active:true};
 try{
  const res=await fetch(U+"/rest/v1/property_alerts",{method:"POST",headers:{apikey:K,"Content-Type":"application/json",Prefer:"return=minimal"},body:JSON.stringify(payload)});
  if(!res.ok){const msg=await res.text();throw new Error(msg||("Supabase "+res.status))}
